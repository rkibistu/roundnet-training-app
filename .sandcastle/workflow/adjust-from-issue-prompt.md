...
## Adjustment issue (describes required changes)

!`gh issue view {{ADJUSTMENT_ISSUE_NUMBER}} --json number,title,body,labels,comments --jq '{number, title, body, labels: [.labels[].name], comments: [.comments[].body]}'`
...
The implementation of issue **#{{ISSUE_NUMBER}}** has been reviewed. Issue **#{{ADJUSTMENT_ISSUE_NUMBER}}** describes the required adjustments — apply them to the existing implementation.
...
1. Read both issues carefully to understand the original intent and what has changed.
...
6. Close the adjustment issue: `gh issue close {{ADJUSTMENT_ISSUE_NUMBER}} --comment "Adjustments applied to branch."`.
