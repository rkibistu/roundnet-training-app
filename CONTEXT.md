...
**Auto-copy attune**: A Player who owns no Domain (or who wants a fresh copy) can Attune without selecting an existing Domain. The system automatically: (1) creates a new Domain for the Player named after the Root Domain (duplicate names are allowed), (2) copies all active Skills from the Root Domain into the new Domain, (3) records the Attunement linking the new Domain to the Root, and (4) auto-pairs every copied Skill to its corresponding Root Skill by exact name match. The new Domain is immediately fully attuned and paired. Archived Skills from the Root Domain are not copied.
